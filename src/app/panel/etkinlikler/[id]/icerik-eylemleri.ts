...
async function gorunurFaaliyet(veri: FormData) {
  ...
  return {
    ...
    yol: `/panel/etkinlikler/${faaliyetId}`,
  };
}
...
function iptalliyseDur(
  ...
  if (!faaliyetIcerikAlabilirMi(faaliyet.durum)) {
    hataylaDon(yol, "Bu etkinlik iptal edildi; yeni içerik eklenemez.");
  }
}
...
export async function ekYukleEylemi(veri: FormData): Promise<void> {
  ...
  if (!ekYukleyebilirMi(kullanici, kapsam)) {
    throw new YetkiHatasi("Bu etkinliğe dosya ekleme yetkiniz yok.");
  }
  ...
  await erisimLogla({
    ...
    hedefId: sonuc.ekId!,
    detay: `Etkinliğe ek yüklendi: ${dosya.name} (${faaliyet.ad})`,
  });
...
export async function kapakSecEylemi(veri: FormData): Promise<void> {
  ...
  if (!ekYukleyebilirMi(kullanici, kapsam)) {
    throw new YetkiHatasi("Bu etkinliğin kapağını değiştirme yetkiniz yok.");
  }
...
export async function ekSilEylemi(veri: FormData): Promise<void> {
  ...
  await erisimLogla({
    ...
    hedefTip: "FAALIYET_EK",
    hedefId: ek.id,
    detay: `Etkinlik eki silindi: ${ek.dosyaAdi}`,
  });
...
export async function yorumYazEylemi(veri: FormData): Promise<void> {
  ...
  if (!yorumYazabilirMi(kullanici, kapsam)) {
    throw new YetkiHatasi("Bu etkinliğe yorum yazma yetkiniz yok.");
  }
  ...
  await erisimLogla({
    ...
    hedefTip: "YORUM",
    hedefId: yorum.id,
    detay: `Etkinliğe yorum yazıldı: ${faaliyet.ad}`,
  });
...
export async function faaliyetPaydasEkleEylemi(veri: FormData): Promise<void> {
  ...
  if (!faaliyetPaydasiYonetebilirMi(kullanici, kapsam)) {
    throw new YetkiHatasi("Bu etkinliğin paydaş bilgisini düzenleyemezsiniz.");
  }
  iptalliyseDur(faaliyet, yol);
  ...
  if (!paydas.aktif) {
    hataylaDon(yol, "Pasife alınmış paydaş yeni etkinliğe bağlanamaz.");
  }
  ...
  if (mevcut) hataylaDon(yol, "Bu paydaş etkinliğe zaten bağlı.");
  ...
  await erisimLogla({
    ...
    hedefTip: "FAALIYET",
    hedefId: faaliyet.id,
    detay: `Etkinliğe paydaş eklendi: ${paydas.ad}`,
  });
...
export async function faaliyetPaydasCikarEylemi(veri: FormData): Promise<void> {
  ...
  if (!bag) hataylaDon(yol, "Bu paydaş etkinliğe bağlı değil.");
  ...
  await erisimLogla({
    ...
    hedefTip: "FAALIYET",
    hedefId: faaliyet.id,
    detay: `Etkinlikten paydaş bağlantısı kaldırıldı: ${bag.paydas.ad}`,
  });
