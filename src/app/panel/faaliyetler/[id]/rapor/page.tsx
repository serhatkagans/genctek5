 ...
 *
 * Görsel ekleme burada TEKRARLANMAZ: faaliyetin ek yükleme kartı zaten var ve
 * aynı yetki kapısını kullanıyor. Buradan oraya yönlendiriliyor, ikinci bir
 * yükleme yolu açmak iki ayrı dosya listesi doğururdu.
 */

const DURUM_MESAJLARI: Record<string, string> = {
  yazildi: "Faaliyet raporu kaydedildi.",
  guncellendi: "Faaliyet raporu güncellendi.",
};
...
export default async function FaaliyetRaporuSayfasi({
  ...
  await erisimLogla({
    ...
    hedefTip: "FAALIYET",
    hedefId: faaliyet.id,
    detay: "Faaliyet raporu ekranı görüntülendi",
  });

  const tekilKatilimci = new Set(basvurular.map((b) => b.katilimciId)).size;
    ...
    <div className="space-y-6">
      ...
      <Kart>
        ...
        <dl className="grid gap-4 sm:grid-cols-3">
          ...
          <div>
            <dt className="text-sm text-metin-yumusak">Katılan</dt>
            <dd className="mt-0.5 text-2xl font-bold text-baslik">
              {basvurular.length}
            </dd>
          </div>
          <div>
            <dt className="text-sm text-metin-yumusak">Farklı kişi</dt>
            <dd className="mt-0.5 text-2xl font-bold text-baslik">
              {tekilKatilimci}
            </dd>
          </div>
      ...
      <Kart>
        <KartBasligi
          ...
          aciklama="Görseller faaliyetin ek listesinde tutulur; rapor onlara işaret eder."
          Ikon={ImagePlus}
        />
        {gorseller.length === 0 ? (
          <p className="text-metin-yumusak">Henüz görsel eklenmemiş.</p>
        ) : (
          <ul className="grid gap-3 sm:grid-cols-3">
            {gorseller.map((gorsel) => (
              <li key={gorsel.id}>
                ...
                <img
                  src={uygulamaYolu(
                    `/panel/faaliyetler/${faaliyet.id}/ekler/${gorsel.id}`,
                  )}
                  alt={gorsel.dosyaAdi}
        ...
        {yazabilir && (
          <Link
            href={`/panel/faaliyetler/${faaliyet.id}#ekler`}
      ...
      <Kart>
        <KartBasligi
          ...
          aciklama={
            rapor
              ...
              : "Faaliyetin nasıl geçtiğini kendi cümlelerinizle yazın."
          }
          Ikon={FileText}
        />
        ...
        {!yazabilir ? (
          rapor ? (
            ...
            <p className="text-metin-yumusak">Bu faaliyetin raporu henüz yazılmadı.</p>
          )
        ) : !hazir.olurMu ? (
      ...
      <Kart>
        ...
        <div className="flex flex-wrap gap-2">
          <a
            href={uygulamaYolu(
              `/panel/faaliyetler/${faaliyet.id}/rapor/indir?bicim=word`,
            )}
          ...
          <a
            href={uygulamaYolu(`/panel/faaliyetler/${faaliyet.id}/rapor/indir`)}
            className={SINIF_IKINCIL_BUTON}
          >
