...
export default async function TopluBelgeSayfasi({
  ...
  const [{ id }, { tur, katilimci, metin }] = await Promise.all([
    params,
    searchParams,
  ]);
  ...
  const belgelerYolu = uygulamaYolu(
    `/panel/faaliyetler/${faaliyet.id}/belgeler`,
  );
  ...
  if (secim.durum !== "hazir") {
    const mesajlar = {
      katilimciYok: {
        baslik: "Bu faaliyete seçilmiş katılımcı yok",
        aciklama:
          "Toplu belge üretebilmek için faaliyete katılımcı seçilmiş olması gerekir.",
      },
      eslesmeYok: {
        baslik: "Seçilen kişiler bu faaliyetin katılımcısı değil",
        aciklama:
          "Adresteki katılımcı kimlikleri bu faaliyetin seçilmiş katılımcılarıyla eşleşmedi.",
      },
      sinirAsildi: {
