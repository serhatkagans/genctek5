...
/**
 * AKIŞ BÖLÜMÜ — "Bağlantılarım" sayfasının içinde (14 Ağustos 2026 · istek:
 * "akış bağlantılarım içine gelecek").
 *
 * KENDİ SEKMESİ VARDI, KALKTI. 12 Ağustos'ta ayrı sekme yapılmıştı ve gerekçe
 * "ikisi farklı şeyler" idi: Bağlantılarım özeldir (iki kişi, danışman onaylı),
 * Akış yayındır (herkes okur, onay yok). Bu ayrım KAYBOLMADI — bölüm kendi
 * başlığı ve kendi kalıcı uyarısıyla (AKIS_UYARISI) basılıyor, yani "kime
 * yazdığını" ekran hâlâ söylüyor. Değişen tek şey, ikisinin aynı sayfada
 * olması: akıştan bağlantı kurulup o bağlantının mesajına geçmek artık sekme
 * değiştirmeyi gerektirmiyor.
 *
 * AYRI DOSYA, sayfanın içine gömülmedi: `/panel/akis` adresi yer imlerinde ve
 * bildirimlerde geçiyordu ve yönlendirmeye dönüştü; bölümün kodu ise eylemleri
 * (akis/eylemler.ts) ve kuralları (lib/akis) ile aynı klasörde kaldı.
 *
 * Aşağıdaki özgün başlık notu 12 Ağustos 2026'dan (istek: "kullanıcı
...
export async function AkisBolumu({
  kullanici,
}: {
  ...
  return (
    <section id="akis" className="scroll-mt-6 space-y-6">
      <div className="flex flex-wrap items-baseline gap-x-3">
        <h2 className="flex items-center gap-2 text-lg font-semibold text-baslik">
          <Newspaper size={18} className="text-vurgu-metin" aria-hidden />
          Akış
        </h2>
        <p className="text-sm text-metin-yumusak">
          Kendini tanıt, çalışmanı paylaş · {gonderiler.length} gönderi
        </p>
      </div>

      {/*
        UYARI BÖLÜMÜN İÇİNDE KALIYOR: sayfanın başındaki uyarı yazışmalar
        içindir ("gizli değildir, danışmanın okur"), buradaki ise yayın olmanın
        kendisini söylüyor. İkisi tek kutuda toplansaydı, aynı ekranda duran iki
        farklı görünürlük kuralı tek cümleye indirgenirdi.
      */}
