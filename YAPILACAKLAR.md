...
5 Ağustos 2026 tarihli istek listesinin planı.

**Durum: 27 maddenin 25'i yapıldı.** A1, C3; cevap beklemeyen beşli (F, B1, D1,
D2, D8); S4 → A2; S6/S13/S17/S18 → B2, B3, C1, J2, J4; S21 → H;
S23/S24/S25 → **J1, J3, J5**; S15 → **D7**; S10/S11 → **D3, D4**; ürün formu
→ **D5**; S14 → **D6**; S7 → **C2**; envanter adları → **E**.
**Profil bölümü (D) tamamlandı** — tek eksik, ürün taahhütnamesinin metni
(→ S12). **E'nin motoru ve üç envanteri hazır**; yayımlanmış dört ölçeğin
madde metinleri bekleniyor (→ S16).
**Kalan 2 madde: C4, G.**

Bir önceki liste (31 Temmuz, `duzeltmeler.txt`) kapandı ve
[`YAPILACAKLAR-ARSIV.md`](YAPILACAKLAR-ARSIV.md) dosyasına taşındı.

Anlaşılmayan ya da çelişkili bulduğum noktalar **[`SORULAR.md`](SORULAR.md)**
dosyasında soru–cevap biçiminde duruyor. Aşağıdaki maddelerde `→ S7` gibi
göndermeler o dosyadaki soru numaralarıdır. **Cevap gelmeden başlanmaması
gereken maddeler ⛔ ile işaretlendi**; işaretsiz maddeler bugünkü bilgiyle
yapılabilir.
...
|---|---|---|---|
| **A** | **Kimlik ve erişim** | | |
| A1 | EBA dışı giriş (mezun, paydaş) + proje yöneticisi onayı | XL | ✅ **YAPILDI** |
| A2 | KVKK/belge akışının menüden kaldırılması | S | ✅ **YAPILDI** |
| **B** | **Adlandırma ve menü düzeni** | | |
| B1 | "Faaliyet" → "Etkinlik" (tüm arayüz) | M | ✅ **YAPILDI** |
| B2 | Sekme ve başlık adları (8 ayrı yeniden adlandırma) | S | ✅ **YAPILDI** |
| B3 | Menüden kaldırılacak sekmeler | S | ✅ **YAPILDI** |
| **C** | **Öğrenci paneli düzeni** | | |
| C1 | Panelim: çalışma grubu + danışman seçimi + katkı girişi | M | ✅ **YAPILDI** |
| C2 | "Başvuru Açık" şeridi → "Mesajın Var" | M | ✅ **YAPILDI** |
| C3 | İlk girişte profil ekranı | XS | ✅ **YAPILDI** |
| C4 | Profil bölümleri için düzenleme/ekleme/silme ekranı | M | ⛔ S9 |
| **D** | **Profil içeriği** | | |
| D1 | "GençTek Yolculuğum" bölümü | S | ✅ **YAPILDI** |
| D2 | "Bilişim Yolculuğum" bölümü | S | ✅ **YAPILDI** |
| D3 | Sertifikalarım | M | ✅ **YAPILDI** |
| D4 | Topluluklarım | M | ✅ **YAPILDI** |
| D5 | Ürünlerim (tanıtım) + "markette paylaş" | M | ✅ **YAPILDI** (taahhütname hariç) |
| D6 | Rotam | M | ✅ **YAPILDI** |
| D7 | Seferlerim (nişan → seviye sistemi) | M | ✅ **YAPILDI** |
| D8 | "Yeni kayıt ekle" düzeltmeleri | M | ✅ **YAPILDI** |
| **E** | **Algoritmam (öz değerlendirme envanterleri)** | XL | ✅ **YAPILDI** (4 envanterin içeriği hariç) |
| **F** | **Danışman seçimi ekranı** | S | ✅ **YAPILDI** |
| **G** | **Mesajlar ve gruplar** | L | ⛔ S19, S20 |
| **H** | **Talep panosu → talep türleri** | M | ✅ **YAPILDI** |
| **I** | **GençTek Market** | L | ✅ **YAPILDI** (DİLİM + moderasyon hariç) |
| **J** | **Öğretmen tarafı** | | |
| J1 | Danışmanlığı bırakma | M | ✅ **YAPILDI** |
| J2 | Görev Rolleri → Öğrencilerim içine | M | ✅ **YAPILDI** |
| J3 | Raporlar + Belge Oluştur → Etkinlikler içine | M | ✅ **YAPILDI** |
| J4 | Paydaşlar → etkinlik ekleme içine | M | ✅ **YAPILDI** |
| J5 | Katılım belgesinde imza makamı | M | ✅ **YAPILDI** |
| **K** | **Sonraki faza bırakılanlar** | — | — |

**Cevaplananlar (5 Ağustos):** S1, S3, S8 → A1, C3; S4 → A2;
**S6, S13, S17, S18 → B2, B3, C1, J2, J4**. S2 cevaplanmadı, varsayımla
ilerlendi (aşağıda A1 · "Yapılan" bölümü); S5 için de varsayım yürürlükte
(Katkılarım sekmesi veri giriş ekranı olarak duruyor).

**Cevaplananlar (6 Ağustos):** S21 → H; S23, S24, S25 → J1, J3, J5;
S10, S11 → D3, D4; S14 → D6; S15 → D7; S7 → C2. **S16 kısmen**: envanterlerin
listesi geldi (→ E yapıldı), madde metinleri hâlâ bekleniyor.
